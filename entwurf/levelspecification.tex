...
Alle diese drei Objekttypen verfügen über eine Zeichenkette mit Namen \strong{"'type"'}, die die Werte
\begin{itemize}
	\item egg
...
Die Ausgangskonstellation findet sich wie bei Mutiple Choice Leveln im Feld \strong{"'initial constellation"'}, und wird mittels eines Board Objektes repräsentiert.
...
Für Ersteres heißt dieses \strong{"'package"'} und beinhaltet eine Ganzzahl größer Null und kleiner hundert.
...
\dirtree{%
.1 "'de{.}croggle"' : Object - Namensraum.
	.2 "'packages"' : Liste - Liste mit Repräsentationen von Paketen{.} Gewöhnlich nur ein Paket.
		.3 : Object - Das zur Datei gehörende Paket als JSON Objekt.
			.4 "'name"' : String - Der Anzeigename des Levelpakets.
			.4 "'description"' : String - Eine Zeichenkette zur Beschreibung des Pakets.
			.4 "'banner"' : String - Pfad zu einer Bilddatei, die als Banner für ein Levelpaket fungiert.
			.4 "'design"' : String - Pfad zu einem Design, das auf alle Levels eines Pakets angewendet wird.
			.4 "'animation"' : String - Pfad zu einer Animation, die vor dem ersten Betreten des Levels abgespielt wird.
			.4 "'dependencies"' : Liste - Die Liste mit Abhängigkeiten.
				.5 "'type"' : String - Der Typ einer Abhängigkeit.
				.5 Weitere Elemente für "'packageprogress"' Abhängigkeiten.
					.6 "'package"' : unsigned Integer - Die Identifikationsnummer eines Pakets als Abhängigkeit.
					.6 "'progress"' : unsigned Integer - Prozentzahl, zu der ein Paket als Abhängigkeit abgeschlossen sein muss.
}
...
\dirtree{%
.1 "'de{.}croggle"' : Object - Namensraum.
	.2 "'levels"' : Liste - Levelobjekte.
		.3 : Object - Darstellung einzelner Levels.
			.4 "'type"' : String - Typ eines Levels.
			.4 "'description"' : String - Beschreibung zu einem Level.
			.4 "'design"' : String - Pfad zu einem Leveldesign.
			.4 "'abort simulation after"' : Integer - Nach wie vielen Schritten ein Level gewonnen (positiv) oder verloren (negativ) ist.
			.4 "'hints"' : Liste - Strings mit Pfaden zu Hilfegrafiken.
			.4 "'data"' (multiple choice) : Object - Spezielle Daten für die einzelnen Level Typen.
				.5 Daten für MC Levels
					.6 "'initial"' : Object - Board Objekt der Anfangskonstellation.
					.6 "'answers"' : Liste - Liste mit Board Objekten der möglichen Antworten.
					.6 "'correct answer"' : Unsigned Integer - Der Index der richtigen Antwort zur Fragestellung.
				.5 Daten für FE Levels.
					.6 "'initial constellation"' : Object - Board Objekt der Anfangskonstellation.
					.6 "'objective"' : Object - Board Objekt der zu erreichenden Konstellation.
					.6 "'blocked colors"' : Liste - Liste mit unsigned Integer Werten, die blockierte Farben beschreiben.
					.6 "'blocked types": Liste - String Liste mit Namen von Elementtypen, die im Level nicht platzierbar sind (egg, colored alligator, aged alligator).
...
\subsubsection{Verschachtelte Alligatorbaumstruktur}
