...
public class FindBoardErrorsTest extends TestCase {
	...
	public void testUncoloredEgg() {
		...
		List<BoardError> result = FindBoardErrors.find(b, new BoardErrorType[]{BoardErrorType.UNCOLORED_OBJECT});
	...
	public void testEmptyAgedAlligator() {
		...
		List<BoardError> result = FindBoardErrors.find(b, new BoardErrorType[]{BoardErrorType.AGEDALLIGATOR_CHILDLESS});
		assertEquals(1, result.size());
		assertEquals(AgedAlligatorChildlessError.class, result.get(0).getClass());
	}
	
	public void testEmptyBoard() {
		List<BoardError> result = FindBoardErrors.find(b, new BoardErrorType[]{BoardErrorType.EMPTY_BOARD});
		assertEquals(1, result.size());
	...
	public void testEmptyColoredAlligator() {
		...
		List<BoardError> result = FindBoardErrors.find(b, new BoardErrorType[]{BoardErrorType.COLOREDALLIGATOR_CHILDLESS});
		assertEquals(1, result.size());
		assertEquals(ColoredAlligatorChildlessError.class, result.get(0).getClass());
	}
